...
- Report detail projections: `domain/reporting/ReportingDetailsUseCase.kt` builds period sales categories and profit buckets plus current customer, inventory-location, and stock-movement rows. The existing reports gateway opens four full report pages in `ui/reporting/ReportingDetailScreen.kt` using the same `ReportingUiState`; no second data owner or persisted shape was introduced.
- Integrations: `data/`
