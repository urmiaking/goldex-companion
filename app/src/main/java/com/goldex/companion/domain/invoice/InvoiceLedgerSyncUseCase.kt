        ...
        if (invoice.customerRole == CustomerRole.WHOLESALER) {
            ...
            if (abs(netWeightToLedger) >= 0.001) {
                ...
                val tx = LedgerTransaction(
                    ...
                    title = if (isPayToCustomer) "فاکتور تهاتر طلا #${invoice.invoiceNumber}" else "بستانکاری فاکتور تهاتر #${invoice.invoiceNumber}",
                    dateTime = dateStr,
                    ...
                    note = "ثبت خودکار از فاکتور تهاتر شماره ${invoice.invoiceNumber}",
            ...
            if (invoice.settlementMethod == SettlementMethod.LEDGER && invoice.ledgerAmount > 0L) {
                val txCash = LedgerTransaction(
                    customerId = customer.id,
                    documentNumber = invoice.invoiceNumber,
                    title = "مانده دفتری فاکتور #${invoice.invoiceNumber} (${invoice.ledgerDueDate})",
                    dateTime = dateStr,
            ...
            if (remainingCash > 0L) {
                ...
                val tx = LedgerTransaction(
                    customerId = customer.id,
                    documentNumber = invoice.invoiceNumber,
                    title = "مانده فاکتور فروش طلا #${invoice.invoiceNumber}",
                    dateTime = dateStr,
                    type = LedgerEntryType.CASH_RIAL,
                    direction = LedgerDirection.PAY,
                    amountTomans = remainingCash,
                    paymentMethod = "مانده دفتری فاکتور",
                    note = "ثبت خودکار مانده پرداخت‌نشده فاکتور ${invoice.invoiceNumber}",
                ...
                val tx = LedgerTransaction(
                    customerId = customer.id,
                    documentNumber = invoice.invoiceNumber,
                    title = "بستانکاری خرید طلا #${invoice.invoiceNumber}",
                    dateTime = dateStr,
                    type = LedgerEntryType.CASH_RIAL,
